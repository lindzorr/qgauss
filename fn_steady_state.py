...
def moment_steadystate(L0: QGsuper,
    ...
    """
    dims = L0.dims_cvs
    A = L0.wigner_2nd_deriv_var
    B = L0.wigner_2nd_var
    C = L0.wigner_2nd_deriv
    D = L0.wigner_1st_var
    F = L0.wigner_1st_deriv
...
def backaction_steadystate(L0: QGsuper,
    ...
    """
    ---- Procedure ----
    Steady-state solver for the backaction rates of a CV system on a qubit, or a CV system coupled to a system of 
    qubits. The function will automatically check if the evolution is Gaussian, and will raise an error if not. The 
    function will also check if a steady-state exists and will exit if not. The component of the qubit state can be 
    specified if the system has an FLS component, and if none is given, the backaction on every component will be 
    solved. If the system is only CV, no qubit state need be specified. The function will return the total dephasing 
    and frequency shift, along with the bare, measurement-induced, and parasitic dephasing subcomponents. 
    ...
    if not L0.isfls and L0.iscvs:
        ba_total,ba_bare,ba_meas_ind,ba_para = _backaction_steadystate_solver(L0, tol)
    ...
    # Qubit state is specified, solve the corresponding CV component
        ...
        if L0.issubgauss(qindex):
            pass
        else:
            sys.exit("Equation of motion for the CV system is not Gaussian. The moment method cannot be used.")

        ba_total,ba_bare,ba_meas_ind,ba_para = _backaction_steadystate_solver(L0[qindex,qindex], tol)
        ...
        for qrow in range(0,row_total):
            for qcol in range(0,col_total): 
                ...
                (ba_total[qrow,qcol], ba_bare[qrow,qcol], ba_meas_ind[qrow,qcol], ba_para[qrow,qcol]) = \
                    _backaction_steadystate_solver(L0[qindex,qindex], tol)
...
def _backaction_steadystate_solver(L0: QGsuper,
    ...
    L0 Liouvillian with governs the dynamics of ρ. The dephasing rate and frequency shift correspond to the real 
    ...
    ba_para : complex
    """
    # Solve the steady-state components of the system evolving under the L0 QGsuper
    ...
    ba_bare = G
    ba_meas_ind = rhof.data_1st @ D + 0.5*rhof.data_1st @ B @ rhof.data_1st
    ba_para = 0.5*np.trace(B @ rhof.data_2nd)
