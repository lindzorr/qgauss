...
def tensor(*args) -> QGstate | QGoper:
    """
    Function handling tensor operation between states (QGstate) or operators 
    (QGoper). Tensor operation for superoperators (QGsuper) is currently not 
    implemented. Both classes can call the tensor operation using the class 
    method "and" (&). Due to the fact that continuous-variable system (CVS) and 
    finite-level system (FLS) components are handled differently, there are 
    certain situation where ordering within tensor will not matter, for example:
        tensor(destroy(),sigmaz()) 
            == tensor(sigmaz(),destroy())
    and
        tensor(destroy(),create(),sigmaz()) 
            == tensor(destroy(),sigmaz(),create()) 
            == tensor(sigmaz(),destroy(),create())
    However, we of course have:
        tensor(destroy(),create()) != tensor(create(),destroy())
    It is recommended to keep ordering consistent even in cases where the 
    tensor function is agnostic.

    ---- Parameters ----
    args : QGstate's or QGoper's
        Arbitrary number of QGstates or QGopers to be tensored together, must 
        all be of same type.
...
def _tensor_oper(args) -> QGoper:
    ...
    # than quadratic/bilinear order in the quadrature basis
