...
def expect(oper: QGoper, 
    ...
    which is FLS only this is the usual trace over a product of matrices:
        expect = tr_FLS[ρ.Q] 
               = tr_FLS[ρ @ Q]
    For a system which is CVS only, the trace is replaced by a product of
    moments of ρ and coefficients of Q:
        expect = tr_CVS[ρ.Q] 
               = Exp[ν]*( ½*tr[Σ.O(2)] + ½*μ.O(2).μ + μ.O(1) + O(0) )
    For a mixed system, the expectation value is calculated by taking the matrix
    product between ρ and Q at the FLS level, and then summing over the 
    different CVS expectation values:
...
def commutator(A: QGoper, 
    ...
    CVS-only; the handling of mixed operators is currently not implemented. If 
    both operators are FLS only then the commuator is simply the usual:
        [A,B]_FLS = A.B - B.A
    If the pair are CVS only operators, then the commutators are evaluated using 
    the known commutation relation for the quadrature operators:
        [r_j,r_k] = i*Ω_jk
...
def ASp_transform(input: QGstate | QGoper | QGsuper,
    ...
    Apply an arbitrary symplectic-affine/canonical transformation to a state, 
    ...
    take the form.
        Q -> ½r.S^T.Q(2).S.r + r.S^T.(Q(2).d + Q(1)) 
             + (½d.Q(2).d + d.Q(1) + O(0))
    The above expressions can be used to apply these transformations to 
    operators and superoperators, taking advantage of the fact that it is 
    ultimately just a unitary transformation. For states, the evolution is:
        ρ -> U.ρ.U* = exp[-i[X,·]].ρ  where  U = exp[-i*X].
    This is equivalent to the following evolution of the moments of ρ(0)
        Σ -> S.Σ.S^T,   
        μ -> S.μ + d
    where the transformations S and d are as before. For objects with an FLS 
    part, the transformation is applied to each sub-component of the 
    total object.
    ...
    gen_oper : QGoper
        QGoper that will act as the generator of the unitary transformations, 
        which is applied as a symplectic-affine transformation on the data 
    ...
    if gen_oper.dims_cvs != input.dims_cvs:
        raise ValueError("CV component of input and generator of the " \
