...
def measurement_rate(H_system: QGoper, 
    ...
    The data in the system-bath Hamiltonian will have the form 
        H_system_bath.data_2nd = [[Hsb,0],[0,Hsb]]. 
    ...
    # measurement rate.
        ...
        q_A = np.prod(H_system.dims_fls[0]) - int(qbinary.split(',')[0],2) - 1
        q_B = np.prod(H_system.dims_fls[1]) - int(qbinary.split(',')[1],2) - 1

        # Generate the output states of the system
        pointer_A = output_state(H_system = H_system[q_A,q_A], 
                                 H_system_bath = H_system_bath, 
                                 input_state = input_state, 
                                 freq = freq)
        pointer_B = output_state(H_system = H_system[q_B,q_B], 
                                 H_system_bath = H_system_bath, 
                                 input_state = input_state, 
                                 freq = freq)
...
def _measurement_rate_solver(pointer_A: QGstate, 
    ...
    if pointer_A == pointer_B:
        ...
        if meas_oper == None:
            ...
            noise_A = meas_oper.data_1st @ pointer_A.data_2nd @ meas_oper.data_1st
            noise_B = meas_oper.data_1st @ pointer_B.data_2nd @ meas_oper.data_1st
            meas_noise = np.real(noise_A + noise_B)
        ...
        signal_A = pointer_A.data_1st @ meas_oper.data_1st
        signal_B = pointer_B.data_1st @ meas_oper.data_1st
        noise_A = meas_oper.data_1st @ pointer_A.data_2nd @ meas_oper.data_1st
        noise_B = meas_oper.data_1st @ pointer_B.data_2nd @ meas_oper.data_1st

        meas_signal = (1/4)*np.abs(signal_A - signal_B)**2
        meas_noise = np.real(noise_A + noise_B)
...
def output_state(H_system: QGoper, 
    ...
    A = (symform_sys @ H_system.data_2nd 
         + (1/2)*symform_sys @ Hsb @ symform_bath @ np.transpose(Hsb))

    s_mat_freq = (-symform_bath 
                  @ np.transpose(Hsb)
                  @ np.linalg.inv(A + 1j*freq*np.identity(2*dims_sys))
                  @ symform_sys 
                  @ Hsb
                  + np.identity(2*dims_bath)
                  )
    s_mat_neg_freq = (-symform_bath 
                      @ np.transpose(Hsb)
                      @ np.linalg.inv(A - 1j*freq*np.identity(2*dims_sys))
                      @ symform_sys 
                      @ Hsb
    ...
    t_mat_freq = (-symform_bath 
                  @ np.transpose(Hsb)
                  @ np.linalg.inv(A + 1j*freq*np.identity(2*dims_sys))
                  )

    output_mean = (s_mat_freq @ input_state.data_1st
                   + t_mat_freq @ symform_sys @ H_system.data_1st
                   )
    output_covariance = (1/2)*(s_mat_freq @ input_state.data_2nd 
                               @ np.transpose(s_mat_neg_freq)
                               + s_mat_neg_freq 
                               @ input_state.data_2nd 
                               @ np.transpose(s_mat_freq)
                               )
